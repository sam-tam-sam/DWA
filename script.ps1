...
try {
  ...
  if ([Threading.Thread]::CurrentThread.ApartmentState -ne 'STA') {
    Write-Log "ERROR: STA mode required! Use: powershell.exe -sta -File script.ps1" 'Red'; return
  }
  ...
  # --- Native P/Invoke (no System.Windows.Forms usage inside C#) ---
  $cSharp = @'
using System;
...
'@
  ...
  # Fixed definition (was syntactically broken)
  ...
  foreach ($lbl in @($titleLabel,$subtitleLabel,$loadingLabel,$percentLabel,$statusLabel)) {
    try { $lbl.UseCompatibleTextRendering=$false } catch {}
  }
  ...
  function Set-CursorVisible([bool]$visible){
    try{
      if ($visible){
        for($i=0;$i -lt 32;$i++){ [void][Native]::ShowCursor($true) }
      } else {
        for($i=0;$i -lt 32;$i++){ [void][Native]::ShowCursor($false) }
      }
    }catch{}
  }
  ...
  #    (After watchdog starts, ESC will be suppressed by the low-level hook.)
